...

    {
            ...
            SqlParameter[] parameters1 = new SqlParameter[]
       {
        ...
        public List<HeThongCanBoModel> GetDanhSachLeTan_TrongCoQuanSuDungPhanMem(int CoQuanID)
        {
           ...
           {
                new SqliteParameter("@CoQuanID",SqlDbType.Int) // id đơn vị sử dụng phần mềm
           };
            parameters[0].Value = CoQuanID;
            try
            {
                using (SqliteDataReader dr = SQLHelper.ExecuteReaderV2(SQLHelper.connectionString, CommandType.Text, QueryManager.Get("v1_HeThong_CanBo_DanhSachLeTan_TrongCoQuanSuDungPhanMem"), parameters))
                {
                    while (dr.Read())
                    {
                        HeThongCanBoModel canBo = new HeThongCanBoModel();
        ...
        public List<HeThongCanBoModel> GetDanhSachLeTan()
        {
            ...
            try
            {
                using (SqliteDataReader dr = SQLHelper.ExecuteReaderV2(SQLHelper.connectionString, CommandType.Text, QueryManager.Get("v1_HeThong_CanBo_GetDanhSachLeTan")))
                {
                    while (dr.Read())
                    {
                        HeThongCanBoModel canBo = new HeThongCanBoModel();
        ...
        public List<HeThongCanBoModel> DanhSachCanBo_TrongCoQuanSuDungPhanMem(int CoQuanID)
        {
            ...
            try
            {
                using (SqliteDataReader dr = SQLHelper.ExecuteReaderV2(SQLHelper.connectionString, CommandType.Text, QueryManager.Get("v1_HeThong_CanBo_DanhSachCanBo_TrongCoQuanSuDungPhanMem"), parameters))
                {
                    while (dr.Read())
                    {
                        HeThongCanBoModel canBo = new HeThongCanBoModel();
